...
**aiFirstGreeting 필드 설명:**
- **정해진 시나리오**: AI가 대화를 시작하는 인사말이 포함됩니다
- **사용자 정의 시나리오**: `null` 값이 반환되며, 사용자가 먼저 대화를 시작해야 합니다
...
### 4. 롤 플레잉 시나리오
- **정해진 시나리오**: AI가 지정된 역할로 대화를 시작합니다 (예: "Welcome to our cafe! What can I get for you today?")
- **사용자 정의 시나리오**: AI 첫 인사가 null이므로 사용자가 먼저 대화를 시작해야 합니다. AI는 지정된 역할로만 응답합니다
- 동적 상황 생성으로 같은 시나리오도 매번 다른 상황이 제공됩니다
- AI는 영어 튜터가 아닌 지정된 역할의 캐릭터로 대화합니다

### 5. AI 응답 특성
- **일반 토픽 세션**: AI는 사용자의 영어 실력에 맞는 난이도로 응답합니다
- **롤 플레잉 세션**: AI는 지정된 역할의 캐릭터로만 응답하며, 영어 튜터 역할을 하지 않습니다
- 중요한 문법 오류나 어휘 오용 시에만 교정을 제안합니다
- 대화의 자연스러운 흐름을 유지합니다
- 롤 플레잉에서는 사용자의 발화에 직접 응답하는 것을 우선으로 합니다
